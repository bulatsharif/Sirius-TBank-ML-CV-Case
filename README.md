...
## Архитектура (бекенд и веб‑приложение)
...
- Модель: `app/models/detect_YOLO.py` — ленивое кэширование `YOLO` из `app/core/config.YOLO_MODEL_PATH`, предикт возвращает боксы `xyxy`, корректная обработка ошибок загрузки.
...
- `Dockerfile` — двухстадийная сборка, изолированное venv, системные библиотеки для OpenCV, запуск через `gunicorn` + `uvicorn.workers.UvicornWorker` от непривилегированного пользователя.
